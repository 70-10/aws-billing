...
function output(serviceBillings) {
    ...
    const diffTotal = serviceBillings
        .map((a) => a.diff)
        .reduce((prev, current) => prev + current);
    const table = new Table({ head: ["Service", "Cost", "Diff from yesterday"] });
    table.push(["Total", util_1.humanizeDollar(totalBilling), `+${util_1.humanizeDollar(diffTotal)}`]);
    serviceBillings.forEach((e) => {
        table.push([e.service_name, util_1.humanizeDollar(e.billing), `+${util_1.humanizeDollar(e.diff)}`]);
    });
